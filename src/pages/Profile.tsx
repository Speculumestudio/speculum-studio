...
const Profile = () => {
  ...
  const save = async () => {
    ...
    if (error) toast.error(error.message); else { toast.success("Saved"); setEditing(false); setProfile({ ...profile, full_name: name, bio, avatar_url: avatar }); }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
          ...
          <div className="flex-1">
            {editing ? (
              <div className="space-y-3 max-w-md">
                ...
                <Textarea value={bio} onChange={(e)=>setBio(e.target.value)} placeholder="Short bio" maxLength={500} />
                <div className="flex gap-2"><Button onClick={save} className="bg-gradient-gold text-primary-foreground">Save</Button><Button variant="ghost" onClick={()=>setEditing(false)}>Cancel</Button></div>
              </div>
            ) : (
              <>
                <h1 className="font-display text-4xl">{profile.full_name ?? "Unnamed"}</h1>
                {profile.bio && <p className="mt-3 text-muted-foreground max-w-xl leading-relaxed">{profile.bio}</p>}
                <div className="mt-5 flex flex-wrap gap-2">
                  <Badge variant="outline">{stats.posts} contributions</Badge>
                  <Badge variant="outline">{stats.completed} lessons completed</Badge>
                  {profile.is_premium && <Badge className="bg-gradient-gold text-primary-foreground">Premium</Badge>}
                </div>
                {isOwn && <Button variant="outline" className="mt-5" onClick={()=>setEditing(true)}>Edit profile</Button>}
              </>
            )}
          </div>
        </div>
      ...
      <section className="container-editorial pb-10">
        <h2 className="font-display text-2xl mb-6">Contributions</h2>
        {posts.length === 0 ? <p className="text-muted-foreground text-sm">No published posts yet.</p> :
      ...
      {isOwn && bookmarks.length > 0 && (
        <section className="container-editorial pb-20">
          <h2 className="font-display text-2xl mb-6">Saved</h2>
